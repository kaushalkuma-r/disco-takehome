...
async def structured(prompt_name: str, out_model: type[T], *, tool: str, model: str | None = None, **inputs) -> tuple[T, CallUsage]:
    ...
    return await structured_raw(text, out_model, tool=tool, model=model)


async def structured_raw(system_text: str, out_model: type[T], *, tool: str, model: str, user_text: str = "Produce the JSON object now.") -> tuple[T, CallUsage]:
    s = get_settings()
